...
class TestFlyDSLRMSNorm(TestCase):
    ...
    def _force_flydsl_perf_wins(self):
        return mock.patch(
            "torch._native.ops.norm.flydsl_rmsnorm_impl._fused_rms_norm_fwd_perf_wins",
            return_value=True,
        )
    ...
    def test_direct_forward_matches_aten_and_reuses_cache(self):
        from torch._native.ops.norm.flydsl_kernels import rmsnorm_cache_info, rmsnorm_fwd

        x, weight = self._make_inputs(16, 128, torch.float16)
        with pn.flydsl.disabled():
            ref_out, _ = torch.ops.aten._fused_rms_norm.default(
                x, [128], weight, EPS
            )
    ...
    def test_direct_forward_matrix_matches_aten(self):
        from torch._native.ops.norm.flydsl_kernels import rmsnorm_cache_info, rmsnorm_fwd
        ...
        for m, n, dtype in configs:
            with self.subTest(m=m, n=n, dtype=dtype):
                x, weight = self._make_inputs(m, n, dtype)

                with pn.flydsl.disabled():
                    ref_out, _ = torch.ops.aten._fused_rms_norm.default(
                        x, [n], weight, EPS
                    )
                got_out, got_rstd = rmsnorm_fwd(x, [n], weight, EPS)

                _assert_close(self, got_out, ref_out, dtype)
                self.assertEqual(got_rstd.dtype, torch.float32)
                self.assertEqual(got_rstd.shape, (m, 1))
                self.assertEqual(got_rstd.device, x.device)

        info = rmsnorm_cache_info()
        self.assertEqual(info["fwd"].misses, len(configs))

    def test_torch_rms_norm_dispatches_to_flydsl(self):
        from torch._native.ops.norm.flydsl_kernels import rmsnorm_cache_info

        x, weight = self._make_dispatch_inputs()
        with pn.flydsl.disabled():
            ref = torch.rms_norm(x, (DISPATCH_N,), weight, EPS)
        with self._force_flydsl_perf_wins():
    ...
    def test_unsupported_eps_falls_back_without_compiling(self):
        ...
        with pn.flydsl.disabled():
            ref = torch.rms_norm(x, (DISPATCH_N,), weight, unsupported_eps)
        with self._force_flydsl_perf_wins():
            got = torch.rms_norm(x, (DISPATCH_N,), weight, unsupported_eps)
    ...
    def test_fused_aten_misaligned_vector_input_falls_back_without_compiling(self):
        storage = torch.randn(
            (DISPATCH_M * DISPATCH_N + 1,), device="cuda", dtype=DISPATCH_DTYPE
        )
        weight_storage = torch.randn(
            (DISPATCH_N + 1,), device="cuda", dtype=DISPATCH_DTYPE
        )
    ...
    def test_user_disable_falls_back_without_compiling(self):
        x, weight = self._make_dispatch_inputs()
        with self._force_flydsl_perf_wins(), pn.flydsl.disabled():
            ref = torch.rms_norm(x, (DISPATCH_N,), weight, EPS)
            got = torch.rms_norm(x, (DISPATCH_N,), weight, EPS)
